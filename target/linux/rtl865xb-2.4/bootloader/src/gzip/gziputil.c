...
#ifdef RCSID
static char rcsid[] = "$Id: gziputil.c,v 1.3 2004/09/02 02:54:06 yjlou Exp $";
#endif
...
extern flash_file ofd;
extern flash_file ifd;

int pt_read(char *outbuf, int n)
{
    int m = 0;
#ifdef _SUPPORT_LARGE_FLASH_
    ...
    if (ifd.read_index == ifd.size)
        return 0;   //end of file

#ifdef _SUPPORT_LARGE_FLASH_
	baseAddr = (uint32)(&ifd.buffer[ifd.read_index]);
 	...
 	if ( isOverlappedBootInst )
	{
	    while(m < n)
	    {
	    	if ( ((uint8*)&ifd.buffer[ifd.read_index]) >= startOver &&
			     ((uint8*)&ifd.buffer[ifd.read_index]) < endOver )
			{
	    		uint8 *p = (uint8*)bdinfo.BackupInst;
				*outbuf++ = p[((uint8*)&ifd.buffer[ifd.read_index])-startOver];
				ifd.read_index++;
			}
			else
			{
				*outbuf++ = ifd.buffer[ifd.read_index++];
			}
...
//    errno = 0;
    do {
        len = pt_read((char*)inbuf+insize, INBUFSIZ-insize);
...
void write_buf(char *buf, unsigned cnt)
{
    char *buf_p = (char *)buf;	
    while (cnt > 0)
    {    	
        ofd.buffer[ofd.write_index++] = *buf_p++;
        cnt--;
    }
    cnt++;
    cnt--;
